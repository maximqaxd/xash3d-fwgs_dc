...
void NET_Init( void )
{
	...
	if (!net_default_dev)
	{
		if(!modem_init())
		{
			Con_Printf( S_ERROR "%s: modem_init failed!\n", __func__);
			return;
		}
		
		ppp_init();
		Con_Printf("%s: Dialing connection\n", __func__);
		int err;
		
		err = ppp_modem_init("11111", 0, NULL);
		if(err != 0) 
		{
			Con_Printf( S_ERROR "%s: Couldn't dial a connection (%d)\n", __func__, err);
			return;
		}
		Con_Printf("%s: Establishing PPP link\n", __func__);
		ppp_set_login("dream", "dreamcast");
		
		err = ppp_connect();
		if(err != 0) 
		{
			Con_Printf( S_ERROR "%s: Couldn't establish PPP link (%d)\n", __func__, err);
			return;
		}
	}
	else
	{
		Con_Printf("%s: BBA found\n", __func__);
	}
#endif
