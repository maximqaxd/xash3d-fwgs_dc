 ...
 * stop moving.
 *
 * The maximum pool size is 8M (Dreamcast) or 16M (Naomi), made up of:
 *
 * - 4096 (8M) or 8192 (16M) blocks of 2k
...
void* alloc_next_available_ex(void* pool, size_t required_size, size_t* start_subblock_out, size_t* required_subblocks_out) {
    ...
    /* Anything gte to block size must be aligned to a block boundary */
                ...
                for(int i = 0; i < 8; ++i) {
                    if((t & 0x80) == 0) {
                        ...
                        if(reset_subblocks) {
                            ...
                            // at a block boundary and this one isn't (i != 0)
                            found_subblocks = 0;
                        } else {
                            found_subblocks++;
                        }
...
int alloc_init(void* pool, size_t size) {
     ...
     * wasn't aligned or we hit BLOCK_COUNT */
    pool_header.pool_size = pool_header.block_count * PVR_RAM_BLOCK_SIZE;

    pool_header.allocations = NULL;

    assert(((uintptr_t) pool_header.base_address) % PVR_RAM_BLOCK_SIZE == 0);
