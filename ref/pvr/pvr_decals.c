/*
pvr_decals.c - decal paste and rendering
...
void R_DecalShoot( int textureIndex, int entityIndex, int modelIndex, vec3_t pos, int flags, float scale )
{
	...
	if( ent && !FBitSet( flags, FDECAL_LOCAL_SPACE ))
	{
		...
		if( !VectorIsNull( ent->angles ))
		{
			...
			shz_xmtrx_rotate_z( ent->angles[1] * (M_PI_F / 180.0f) );
			shz_xmtrx_rotate_y( ent->angles[0] * (M_PI_F / 180.0f) );
			shz_xmtrx_rotate_x( ent->angles[2] * (M_PI_F / 180.0f) );
			shz_xmtrx_translate( ent->origin[0], ent->origin[1], ent->origin[2] );
			shz_xmtrx_store_4x4( &matrix );
			PVR_Mat4x4_VectorITransform( &matrix, pos, pos_l );
		}
		else
		{
