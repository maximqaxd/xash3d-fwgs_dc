/*
pvr_rlight.c - dynamic and static lights
...
static colorVec R_LightVecInternal( const vec3_t start, const vec3_t end, vec3_t lspot, vec3_t lvec )
{
	...
	if( WORLDMODEL && WORLDMODEL->lightdata )
	{
		...
		for( i = 0; i < maxEnts; i++ )
		{
			...
			{
				// build object matrix (no scale) and inverse-transform to local space
				shz_xmtrx_init_identity();
				shz_xmtrx_rotate_z( pe->angles[1] * (M_PI_F / 180.0f) );
				shz_xmtrx_rotate_y( pe->angles[0] * (M_PI_F / 180.0f) );
				shz_xmtrx_rotate_x( pe->angles[2] * (M_PI_F / 180.0f) );
				shz_xmtrx_translate( offset[0], offset[1], offset[2] );
				shz_xmtrx_store_4x4( &matrix );

				PVR_Mat4x4_VectorITransform( &matrix, start, start_l );
				PVR_Mat4x4_VectorITransform( &matrix, end, end_l );
			}
