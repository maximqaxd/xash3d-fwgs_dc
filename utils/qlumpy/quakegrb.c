...
void GrabRaw(void)
{
    ...
    // Check if we're dealing with PVR data
    if (byteimage && *(uint32_t*)byteimage == PVRTSIGN) {
        fflush(stdout);
        
        pvrHeader = (pvrt_t*)byteimage;
        ...
        // Verify it's a VQ texture
        if (pvrHeader->imageFormat != 0x03) {
            Error("PVR texture must be VQ compressed (format = 0x%02x)", 
                  pvrHeader->imageFormat);
        }

        // Calculate VQ data size
        int vqDataSize = 2048 + ((w * h) / 4);
        
        // Copy PVR header
        memcpy(lump_p, pvrHeader, sizeof(pvrt_t));
        lump_p += sizeof(pvrt_t);
        
        // Copy VQ data (codebook + indices)
        memcpy(lump_p, byteimage + sizeof(pvrt_t), vqDataSize);
        lump_p += vqDataSize;
...
void GrabMip(void)
{
    ...
    byte            *screen_p = NULL, *source, testpixel;
    int             linedelta;
    miptex_t        *qtex;
    ...
    int             count;
    pvrt_t          *pvrHeader;
    qboolean        isPVR = false;


    fflush(stdout);
    
    if (!byteimage) {
        Error("GrabMip: No image loaded (byteimage is NULL)");
    }
    
    if (!lump_p) {
        Error("GrabMip: No lump buffer initialized (lump_p is NULL)");
    }
	...
	// Check if the texture is a PVR file
	if (*(uint32_t*)byteimage == GBIXHEADER || *(uint32_t*)byteimage == PVRTSIGN) {
		isPVR = true;
		if (*(uint32_t*)byteimage == GBIXHEADER) {
			// Handle GBIX header if present
			gbix_t* gbix = (gbix_t*)byteimage;
			pvrHeader = (pvrt_t*)((byte*)byteimage + sizeof(gbix_t) + gbix->nextTagOffset);
		}
		else {
			// Direct PVR header
			pvrHeader = (pvrt_t*)byteimage;
		...
		// Validate supported PVR formats
		if (pvrHeader->imageFormat != PVR_VQ && pvrHeader->imageFormat != PVR_VQ_MIPMAP &&
			pvrHeader->imageFormat != PVR_SMALL_VQ && pvrHeader->imageFormat != PVR_SMALL_VQ_MIPMAP &&
			(pvrHeader->colorFormat != PVR_ARGB1555 && pvrHeader->colorFormat != PVR_RGB565)) {
			Error("Unsupported PVR format: image=0x%02X, color=0x%02X", pvrHeader->imageFormat, pvrHeader->colorFormat);
		}
    ...
    // Generate mipmaps for standard textures
