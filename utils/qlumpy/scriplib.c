...
qboolean EndOfScript (qboolean crossline)
{
    ...
    // Check if script and buffer are valid before freeing
    if (script && script->buffer) {
        free(script->buffer);
        script->buffer = NULL;  // Clear the pointer after freeing
    }
...
//
    ...
    {
        // Handle negative numbers and regular tokens
        while (*script->script_p > 32 && *script->script_p != ';')
        {
            ...
            {
                *token_p++ = *script->script_p++;
                // After a minus sign, we need at least one digit
                if (*script->script_p < '0' || *script->script_p > '9')
                {
                    break;  // Not a negative number, treat as separator
                }
            }
            else
            {
