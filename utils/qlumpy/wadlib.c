...
static char wadfilename[1024];  // Store WAD filename for reopening if needed
...
{
	printf("DEBUG: NewWad: Creating %s\n", pathname);
    fflush(stdout);
    
    strncpy(wadfilename, pathname, sizeof(wadfilename)-1);
    wadfilename[sizeof(wadfilename)-1] = 0;
    
    outwad = fopen(pathname, "wb");
    if (!outwad) {
        Error("NewWad: Cannot create %s (errno: %d)", pathname, errno);
    }
