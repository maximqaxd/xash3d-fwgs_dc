...
void LoadScreenPVR(char* name) {
	char* expanded = ExpandPathAndArchive(name);
	FILE* f = fopen(expanded, "rb");
	long fileSize;

	if (!f) Error("Couldn't open %s", expanded);

	fseek(f, 0, SEEK_END);
	fileSize = ftell(f);
	fseek(f, 0, SEEK_SET);

	if (byteimage) free(byteimage);
	byteimage = malloc(fileSize);
	if (!byteimage) Error("Failed to allocate %ld bytes for PVR file", fileSize);

	if (fread(byteimage, 1, fileSize, f) != fileSize) Error("Failed to read PVR file");
	fclose(f);
	...
	if (pvrHeader->version != PVRTSIGN) Error("%s is not a valid PVR file", expanded);
	...
	if (pvrHeader->imageFormat != PVR_VQ && pvrHeader->imageFormat != PVR_VQ_MIPMAP &&
		...
		Error("%s has unsupported PVR format (image: 0x%02X, color: 0x%02X)",
			expanded, pvrHeader->imageFormat, pvrHeader->colorFormat);
