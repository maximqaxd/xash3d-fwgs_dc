...
void WriteTexturesPVR()
{
    ...
    for (i = 0; i < numtextures; i++) {
        ...
        if (pvr_file) {
            ...
            size_t pvr_size = ftell(pvr_file);
            fseek(pvr_file, 0, SEEK_SET);

            // Read and write the entire PVR file (including header)
            fread(pData, 1, pvr_size, pvr_file);
            pData += pvr_size;
            
            printf("Writing PVR texture %s: %dx%d (%zu bytes)\n", 
                   texture[i].name, 
                   ptexture[i].width, 
                   ptexture[i].height,
                   pvr_size);
...
//
	...
	printf( "=============================================\n");
	printf( "PVRStudioMDL Version.1.0 by OVERRiDE 10-2010\n" );
	printf( "=============================================\n");
}
