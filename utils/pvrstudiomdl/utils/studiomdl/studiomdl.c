...
void Grab_BMP(char *filename, s_texture_t *ptexture)
{
    ...
    // LoadPVR returns 0 on success, -1 on failure
    if ((result = LoadPVR(filename)) < 0) {
        Error("Error reading PVR texture \"%s\"\n", filename);
    }

    printf("Successfully loaded PVR texture: %s (%dx%d)\n", 
           filename, bmhd.w, bmhd.h);
...
void ResizeTexture( s_texture_t *ptexture )
{
	...
	//I have commented out Valve's original code to calculate .bmp texture size
	//ptexture->size = ptexture->skinwidth * ptexture->skinheight + 256 * 3;

    /*And I have added code to calculate .pvr texture size, assuming VQ(4) Non-Mip-Mapped texture
	if ( ptexture->skinwidth == 512 & ptexture->skinheight == 512 ) {
		ptexture->size = 67616;
	}

	else if ( ptexture->skinwidth == 256 & ptexture->skinheight == 256) {
		ptexture->size = 18464;
	}

	else if ( ptexture->skinwidth == 128 & ptexture->skinheight == 128 ) {
		ptexture->size = 6176;
	}
	else if ( ptexture->skinwidth == 64 & ptexture->skinheight == 64 ) {
		ptexture->size = 3104;
	}
	else if ( ptexture->skinwidth == 32 & ptexture->skinheight == 32 ) {
		ptexture->size = 2080;
	}
	else {
		Error( "Error: [%dx%d] Invalid PVR Resolution.\n", ptexture->skinwidth, ptexture->skinheight );
		return -1;
	} */
  
    printf ("%s [%dx%d]\n", ptexture->name,  ptexture->skinwidth, ptexture->skinheight );

	// Allocate 0 bytes for textures; we will apend them at the end
	ptexture->size = 0;
...
void Grab_Skin ( s_texture_t *ptexture )
{
	...
	if (stricmp( ".pvr", &file1[strlen(file1)-4]) == 0) {
